...
export function asApiResponse<T>(
  request: RequestOptions,
  response: IncomingMessage,
  resolve: (result: ApiResponse<T>) => void
): (result: T) => void {
 ...
 */
export function apiRequest<T, O>(
  buildRequest: build.RequestBuilder<T>,
  handleRequest: request.RequestHandler<T>,
    ...
    return new Promise((resolve, reject) => {
      handleRequest(
        ...
        createRequest(options, (response) =>
          handleResponse(
            response,
            asApiResponse(options, response, resolve),
            reject
          )
        ).on("error", reject)
      );
...
export function getJson<T>(
  ...
  return apiRequest(buildRequest, request.emptyBody, response.decodeJson);
}
...
export function sendURLEncoded<T, O>(
  ...
  return urlEncodedRequest(buildRequest, encoder, responseHandler);
}
...
function mapBuffer<O, T extends string, S extends string>(
  encoder: (options: O) => T,
  contentType: S
) {
...
export function requireSuccess<T>(
  ...
  if (response.response.statusCode >= 300) {
    return Promise.reject(
      new Error(
        `Failed: ${response.request.method} ${response.request.host}${
          response.request.path
        } ${response.response.statusCode} ${JSON.stringify(response.result)}`
      )
    );
  }
...
export function requireStatusCode<T>(
  statusCode: number,
  ...statusCodes: number[]
): (response: ApiResponse<T>) => Promise<ApiResponse<T>> {
  const acceptedCodes: { [code: number]: true | undefined } = [
    statusCode,
    ...statusCodes,
  ].reduce((codes, code) => ({ ...codes, [code]: true }), {});
  return (response) => {
    ...
    if (!acceptedCodes[responseStatusCode]) {
      return Promise.reject(
        new Error(
          `Unexpected status code: ${responseStatusCode} ${response.request.method} ${response.request.host}${response.request.path}`
        )
      );
    }
    return Promise.resolve(response);
...
export function requireValidResponse<T>(
  response: ApiResponse<Result<T>>
): Promise<T> {
